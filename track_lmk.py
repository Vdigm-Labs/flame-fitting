...
with torch.no_grad():
    img_np = np.asarray(crop_pil)
    tf_out = transform(img_np)

    if isinstance(tf_out, dict) and "image" in tf_out:
        input_tensor = tf_out["image"]
    else:
        input_tensor = tf_out

    print(f"[디버그] transform 출력 shape: {input_tensor.shape}")
